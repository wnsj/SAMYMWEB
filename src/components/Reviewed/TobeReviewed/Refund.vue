...
<template>
	...
	<div class="wraper">
		...
		<div class="row newRow">
			...
			<button type="button" class="btn btn-primary pull-right m_r_10 jh-mr-2" data-toggle="modal" v-on:click="getRefundApproveFind(1)">查询
			</button>
		</div>
		...
		methods: {
			...
			btnAction(index) {
				...
				switch (index) {
					...
					//舍弃
					case '3':
						this.initData(this.refundId);
						break;
				}
			},
			
			xiaofei() {
				var url = this.url + '/refundAuditBean/refundDiscard'
