			...
			<div class="col-md-10 col-md-offset-1">
				<div class="form-group">
					...
					<div class="col-md-9" style="line-height: 34px;">
						<input class="form-control" type="text" name="" placeholder="科室名称" v-model="departmentName" />
					</div>
				</div>
			</div>
			<div class="col-md-10 col-md-offset-1">
...
<script>
	export default {
		...
		methods: {
			// Initialization department’s content
			initData(param,department) {
				if(param=='add'){
					console.log('Initialization department’s content, which adds department')
					this.departmentName=''
					this.type='add'
					this.title='新增'
					this.isUse=true
					...
					console.log('Initialization department’s content, which modifies department')
					this.type='modify'
					this.title='修改'
					this.departmentName=department.name
					this.deptId=department.deptId
					if(department.isuse=='1'){
						this.isUse=true
						this.notUse=false
					}else{
						this.notUse=true
			...
			addDepartment(){
				...
				if(this.isBlank(this.departmentName)){
					alert('添加的科室的名字不能为空')
					return
				}
				if(this.type=='add'){
					var url = this.url + '/departmentAction/addDepartment'
				}else if(this.type=='modify'){
					var url = this.url + '/departmentAction/updateDepartment'
				}
				var is
				if(this.isUse==true){
					is='1'
