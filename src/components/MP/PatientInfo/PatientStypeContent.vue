<template>
	<div class="modal-content">
		...
		<div class="modal-footer">
			<button class="btn btn-primary" @click="addObj()">确认</button>
		</div>
	</div>
</template>
		...
		methods: {
			...
			initData(param,obj) {
				if(param=='add'){
					...
					this.type='modify'
					this.title='修改'
					this.objName=obj.patitypename
					this.objId=obj.patitypeid
					if(obj.isuse=='1'){
						this.isUse=true
						this.notUse=false
					}else{
			...
			addObj(){
				...
				if(this.type=='add'){
					var url = this.url + '/patienttypeAction/addPatientType'
				}else if(this.type=='modify'){
					var url = this.url + '/patienttypeAction/updatePatientType'
				}
				var is
				if(this.isUse==true){
					is='1'
				...
				this.$ajax({
					...
					data: {
						patitypeid:this.objId,
						patitypename:this.objName,
						isuse:is,
					},
					dataType: 'json',
				}).then((response) => {
